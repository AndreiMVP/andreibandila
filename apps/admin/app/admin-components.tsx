...
export function AboutEditor({ about, setAbout, save, uploadPortrait, removePortrait, sections, setSections, newSection, saveSections }: { about: About; setAbout: (about: About) => void; save: () => void; uploadPortrait: (file?: File) => void; removePortrait: () => void; sections: AboutSection[]; setSections: (sections: AboutSection[]) => void; newSection: () => void; saveSections: () => void }) {
  return <div><div className="admin-toolbar"><h2>Despre</h2><div className="admin-toolbar-actions"><button onClick={save}><Save size={15} />Salvează bio</button></div></div><div className="admin-fields admin-grid"><ImageControl label="Imagine portret" src={about.portrait_image} alt="Portret" onUpload={uploadPortrait} onRemove={removePortrait} /><PlainTextField label="Conținut text" value={about.content} onChange={(v) => setAbout({ ...about, content: v })} /></div><div className="admin-toolbar"><h2>Mini-secțiuni</h2><div className="admin-toolbar-actions"><button type="button" onClick={newSection}><Plus size={15} />Secțiune</button><button type="button" onClick={saveSections}><Save size={15} />Salvează secțiuni</button></div></div><AboutSections sections={sections} setSections={setSections} /></div>;
}
